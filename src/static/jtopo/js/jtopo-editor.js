 ...
 */
import {generateUUID, Timer, topoImgPath} from "@/static/jtopo/js/util";
 ...
 * 获取序列化的拓扑图JSON数据
 */
TopologyPanel.prototype.getSavedTopology = function () {
...
TopologyEditor.prototype.init = function (topologyGuid, backImg, topologyJson) {
    ...
    // 双击事件 放置于Vue文件中mounted里了
    // this.scene.dbclick(function (event) {
    //     console.log('dbclick! tag! ', event)
    //     if (event.target) {
    //         self.currentNode = event.target
    //     } else {
    //         // 双击了舞台空白处
        ...
        if (event.button === 2) {                      // 右键菜单
            ...
            if (event.target) {
                if (event.target instanceof JTopo.Node) {          // 处理节点右键菜单事件
                    ...
                    // console.log('mouseup target line', self.lineMenu, $('#line-menu'))
                    self.lineMenu.css({
...
TopologyEditor.prototype.drag = function (modeDiv, drawArea, text) {
    ...
    modeDiv.ondragstart = function (event) {
        ...
        // 此处 Vue会将图片src中加上一串莫名其妙的混淆，需要特殊处理
        if(backImg.indexOf('.') !== backImg.lastIndexOf('.')) {
            backImg = backImg.substring(0, backImg.indexOf('.')) + backImg.substring(backImg.lastIndexOf('.'))
        }
        // 特殊处理完成
    ...
    drawArea.ondrop = function (event) {
        ...
        if (data) {
            ...
            if (datas && datas.length === 3) {
                ...
                if (nodeType === 'TEXT') {
                    // 创建文本编辑框
                    let msgNode = new JTopo.TextNode("双击编辑文字, 点击节点可连线(连个节点)")
                    msgNode.fontColor = self.config.nodeFontColor
                    msgNode.zIndex++
                    // 节点坐标
                    msgNode.setBound((event.layerX ? event.layerX : event.offsetX) - self.scene.translateX - self.config.nodeDefaultWidth / 2, (event.layerY ? event.layerY : event.offsetY) - self.scene.translateY - self.config.nodeDefaultHeight / 2, self.config.nodeDefaultWidth, self.config.nodeDefaultHeight)
                    // 节点数据
                    msgNode.nodeId = generateUUID()
                    msgNode.nodeType = nodeType
                    self.scene.add(msgNode)
                    self.currentNode = msgNode
                } else {
...
// 编辑器实例
export let editor = new TopologyEditor('mainControl')
...
editor.utils = {
    ...
    // 删除连线
    deleteLine: function () {
        if (editor.currentNode == null && editor.scene.selectedElements.length > 0) {
            editor.currentNode = editor.scene.selectedElements[0]
        }
        // console.log('deleteLine', editor.currentNode)
