!function (window) {
	// const topoImgPath = "../static/jtopo/img/";
	const {topoImgPath} = require("@/static/jtopo/js/util");
	...
	const config = {
		...
		nodeFontColor: '223,223,255',               // 节点文字颜色,如"255,255,0"
	...
	CanvasRenderingContext2D.prototype.JTopoRoundRect = function (a, b, c, d, e) {
		...
		setupScenes: function (scenes, b) {
			scenes.forEach(function (a) {
				...
				d.forEach(function (a) {
					...
					if ("node" == d) {
						c = new JTopo.Node;
						c.alpha = config.nodeAlpha;
						c.strokeColor = config.nodeStrokeColor;
						c.fillColor = config.nodeFillColor;
						c.shadow = config.nodeShadow;
						c.shadowColor = config.nodeShadowColor;
						c.font = config.nodeFont;
						c.fontColor = config.nodeFontColor;
						c.borderRadius = null;
					...
					if ("link" == d) {
						...
						if (nodeA && nodeZ) {    // 折线和直线绘制
							...
							if (a["lineType"] == "foldLine") {
								...
								c.bundleOffset = config.linkOffsetGap;
							}
							if (a["lineType"] == "flexLine") {
								c = new JTopo.FlexionalLink(nodeA, nodeZ);
								c.offsetGap = config.linkOffsetGap;
							}
							if (a["lineType"] == "curveLine") {
								c = new JTopo.CurveLink(nodeA, nodeZ);
							}
							c.alpha = config.linkAlpha;
							c.strokeColor = config.linkStrokeColor;
							c.fillColor = config.linkFillColor;
							c.shadow = config.linkShadow;
							c.shadowColor = config.linkShadowColor;
							c.font = config.linkFont;
							c.fontColor = config.linkFontColor;
							c.arrowsRadius = config.linkArrowsRadius;
							c.lineWidth = config.linkDefaultWidth;
							c.shadowOffsetX = config.shadowOffsetX;
							c.shadowOffsetY = config.shadowOffsetY;
						}
					} else if ("container" == d) { // 增加分组容器
						if (a["childNodes"]) {
							var temp = a["childNodes"].split(",");
							if (temp && temp.length > 0) {
								c = new JTopo.Container();
								c.textPosition = 'Top_Center'
								c.fontColor = config.nodeFontColor
								// container.borderColor = config.nodeBorderColor
								c.borderColor = config.nodeBorderColor
								c.borderRadius = config.nodeBorderRadius
								// c.alpha = config.containerAlpha;
								// c.strokeColor = config.containerStrokeColor;
								// c.fillColor = config.containerFillColor;
								// c.shadow = config.containerShadow;
								// c.shadowColor = config.containerShadowColor;
								// c.font = config.containerFont;
								// c.fontColor = config.containerFontColor;
								// c.borderColor = config.containerBorderColor;
								// c.borderRadius = config.containerBorderRadius;
								// c.shadowOffsetX = config.shadowOffsetX;
								// c.shadowOffsetY = config.shadowOffsetY;
		...
		replaceStageWithJson: function (jsonObj) {
			...
			this.setupScenes(scenes, editor.scene)*/
		},
		createStageFromJson: function (jsonObj, canvas) {
			...
			var scenes = jsonObj.childs;
			this.setupScenes(scenes, new JTopo.Scene(stage))
			return stage;
		}
